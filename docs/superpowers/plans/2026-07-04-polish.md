...
- The dictation pipeline is in daily use — do not modify `ReplacementProcessor`, `WhitespaceCleanupProcessor`, or transcription code. `SnippetProcessor` may be refactored internally to add new entry points (Task 4), but its existing `process(_:)` behavior and every existing SnippetProcessor test must be preserved unchanged.
